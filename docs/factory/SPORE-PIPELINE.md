...
> **鐵律**：孢子 draft 寫完後，**不得直接交給觀察者**。必須先跑 `check-manifesto-11.sh` 抓 9 種「不是X是Y」對位變體 + 破折號密度。任何違反都要改寫或顯式確認保留（引語、特殊場景除外）。
...
- 真正改寫：**「這筆捐款是 3,401 萬的三分之一 — 一個普通人難以想像的重量。」**（用數字+描述取代對位）
