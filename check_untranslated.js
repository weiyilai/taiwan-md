...
// 讀取翻譯映射
const translationsPath = path.join(process.env.HOME, 'taiwan-md/knowledge/_translations.json');
    ...
    for (const entry of entries) {
        ...
        if (stat.isDirectory()) {
            ...
            if (!entry.startsWith('_') && !entry.includes('ROADMAP')) {
                const relativePath = path.relative(path.join(process.env.HOME, 'taiwan-md/knowledge'), fullPath);
...
// 按優先序排序分類
const priorityOrder = ['Art', 'Culture', 'Technology', 'Economy', 'Nature', 'Society', 'Geography', 'Music', 'Food', 'Lifestyle', 'People', 'About'];
